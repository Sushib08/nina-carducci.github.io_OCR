(function ($) {
  $.fn.mauGallery = function (options) {
    ...
    var tagsCollection = [];
    var lightboxId = options.lightboxId || "galleryLightbox";
    ...
    return this.each(function () {
      ...
      $.fn.mauGallery.listeners(options, lightboxId);
  ...
  $.fn.mauGallery.listeners = function (options, lightboxId) {
  ...
  $.fn.mauGallery.methods = {
    ...
    prevImage(lightboxId) {
      ...
      if (activeTag === "all") {
        $(".item-column img").each(function (index) {
        ...
        $(".item-column img").each(function (index) {
          if ($(this).data("gallery-tag") === activeTag) {
            imagesCollection.push($(this).attr("src"));
            if ($(this).attr("src") === activeImage) {
              currentIndex = imagesCollection.length - 1;
            }
          }
        });
      }
      ...
      if (currentIndex > 0) {
        $(`#${lightboxId}`)
          .find(".lightboxImage")
          .attr("src", imagesCollection[currentIndex - 1]);
      }
    },
    ...
    nextImage(lightboxId) {
      ...
      if (currentIndex < imagesCollection.length - 1) {
        $(`#${lightboxId}`)
          .find(".lightboxImage")
          .attr("src", imagesCollection[currentIndex + 1]);
      }
    ...
    filterByTag() {
      ...
      if (tag === "all") {
        ...
        $(".item-column img[data-gallery-tag='" + tag + "']")
          .parents(".item-column")
          .show(300);
      }
    },
